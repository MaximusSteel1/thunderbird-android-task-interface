...
| detail `session-action` 当前主写路径 | 已有 focused facade sender、ViewModel、screen、codec coverage；当前已覆盖 plain reply、quick answer、structured `Answers`、`attachment_continuation`、`/status`、`/kill`、`/end`；submit 成功后会写入 `pending submission`，并按 `session_snapshot.latest_session_action.command_id` 清理 | 仍缺 fresh device / VPS live smoke；`paused` / `resume` 目前尚未暴露 dedicated public control |
...
| `paused` session detail 读法 | 已有 focused ViewModel / screen coverage；当前 paused session 会显示 reply unavailable，而不会在 detail 页自动 prepend `/resume` | dedicated `pause` / `resume` public control 仍待后续切片承接 |
...
.\\gradlew.bat :feature:taskmail:internal:testDebugUnitTest --tests "net.thunderbird.feature.taskmail.internal.ui.detail.TaskSessionDetailViewModelTest" --tests "net.thunderbird.feature.taskmail.internal.data.facade.OkHttpTaskMailSessionActionFacadeSenderTest" --tests "net.thunderbird.feature.taskmail.internal.data.cache.TaskSessionDetailJsonCodecTest" --tests "net.thunderbird.feature.taskmail.internal.ui.detail.TaskSessionDetailScreenKtTest" --console=plain
...
- `session-action`：需要补 fresh device / VPS live smoke，确认 `pending submission -> latest_session_action.command_id closeout -> detail` 真机闭环
...
- paused / resume：detail public UI 还没有 dedicated control，这一块不能误写成已完成切换
