...
- for steady-state Task list refresh, prefer a small recent-message window instead of broad default mailbox scans
...
For Task list / workspace refresh in steady state, step 5 should use a recent-window strategy instead of re-reading the
default broad mailbox page on every refresh.
...
## 7.5 Fetch window strategy

Once local `UnifiedMessage` persistence exists, Task list / workspace refresh should no longer depend on broad default
mailbox fetch sizes such as 100 or 250 messages.

Recommended first-phase strategy:

- steady-state refresh:
  - read local cache first
  - fetch a small recent raw-message window
  - apply cursor-based incremental sync on top of that window
- recommended initial steady-state recent window:
  - `10` raw messages
- bootstrap / recovery mode:
  - allow a wider backfill window only when needed
  - examples:
    - first install
    - cache cleared
    - cursor missing
    - parser version reset
    - detected local/remote divergence that requires repair

Important distinction:

- `10` is the **steady-state refresh window**, not a permanent universal fetch ceiling
- wider backfill remains a controlled fallback path, not the default path
- the benefit of local persistence is that normal refresh stops rebuilding Task state from a broad mailbox window every time
...
- [ ] Implement steady-state Task list refresh with a recent raw-message window of `10`
- [ ] Add bootstrap/recovery fallback backfill rules instead of always relying on the default broad mailbox fetch size
...
8. Normal Task list refresh no longer depends on broad default mailbox fetch windows such as 100 or 250 messages.
9. Future relay support can be added by implementing new transport / ingress / parser modules rather than rewriting business and UI layers.
