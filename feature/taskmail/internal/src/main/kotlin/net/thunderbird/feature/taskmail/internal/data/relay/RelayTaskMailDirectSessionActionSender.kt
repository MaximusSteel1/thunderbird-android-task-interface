...
internal class RelayTaskMailDirectSessionActionSender(
    ...
    override suspend fun send(
        ...
        return relayConnectionClient.sendPacket(buildPacket(request, requestId)).fold(
            ...
            onFailure = { error ->
                error.toDirectSessionActionResult()
            },
        )
    }
...
private fun Throwable.toDirectSessionActionResult(): TaskMailDirectSessionActionResult {
    return when (this) {
        is RelayServerException -> {
            if (code in HARD_REJECTION_CODES) {
                TaskMailDirectSessionActionResult.Rejected(message)
            } else {
                TaskMailDirectSessionActionResult.FallbackToMail(message)
            }
        }

        else -> TaskMailDirectSessionActionResult.FallbackToMail(message)
    }
}
...
private fun RelayPacketAck.toDirectSessionActionResult(
    ...
    return if (accepted) {
        ...
        if (relayErrorCode in HARD_REJECTION_CODES) {
            ...
            TaskMailDirectSessionActionResult.FallbackToMail(errorMessage)
        }
    }
}
